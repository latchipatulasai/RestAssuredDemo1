...
Examples:Transparency apis is an examples in ASH
 ...
 # Session 9
 # Framework Desiging 
The main object of framwork is -maintain all project related files
maintainability
readability
reusability

There are different types of framework are there ,we mainly focused on hybrid -means combination of any two frame works

for developing framework ,we need to follow some phases
1)Understanding requirement
   -Fucntional specifications(static)
   -Swagger
2)Choose automation tool -restassured
restassured,selnium are not tools -these are liabraies
3)Design 
 -folder structure,types of files required
4)Development
5)Execution + CI
